...
import ThemeSwitcherRound from "./theme-switcher-round";
  ...
  return (
    <>
      <div
        ...
        className={`relative flex items-center rounded-full border-[6px] border-background bg-highlight py-2 dark:bg-blue-200 max-lg:hidden ${theme === "light" ? "shadow-inner-right" : "shadow-inner-left"}`}
      >
        <div
          onClick={() => toggleTheme()}
          style={{ height }}
          className={`absolute left-0 top-0 z-20 h-full w-[52%] cursor-pointer rounded-full bg-background transition-transform duration-300 ${
            theme === "light"
              ? "translate-x-0 shadow-right-down"
              : "translate-x-full shadow-left-up"
          }`}
        />
        <button
