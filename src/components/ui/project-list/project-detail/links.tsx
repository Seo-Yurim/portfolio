  ...
  return (
    <div className="flex flex-col gap-4">
      <div className="flex items-center gap-2 border-b-2 border-blue-200 dark:border-white">
        <IoDocumentsSharp className="h-5 w-5 max-md:h-4 max-md:w-4" />
        <h2 className="text-xl font-bold max-md:text-base">서비스 관련 자료</h2>
      </div>
      ...
      <div className="flex items-center gap-8 max-md:gap-4">
        {links.map((link, idx) => (
          ...
          >
            <div className="group relative w-fit rounded-full border-2 border-blue-200 p-2 transition-colors duration-300 hover:bg-blue-200 hover:text-white max-md:p-1">
              ...
              <div className="absolute left-1/2 top-full mt-2 -translate-x-1/2 whitespace-nowrap rounded-full bg-blue-200 px-4 py-1 opacity-0 transition-opacity duration-300 group-hover:opacity-100">
                <p className="text-white max-md:text-sm">{link.label}</p>
              </div>
            </div>
          </Link>
        ))}
